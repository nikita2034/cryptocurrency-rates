...
function CryptocurrencyPage() {
 const {item,portfolioOfCurrencies,setPortfolioOfCurrencies}=useContext(ItemContext);
 const [AmountOfCurrency,setAmountOfCurrency]=useState<number>(0);
  ...
  function onClickButton(){
    if(AmountOfCurrency!==0){
      setPortfolioOfCurrencies([...portfolioOfCurrencies,{name:item.name,price:item.priceUsd,AmountOfCurrency}]);
    }
    ...
    <div className="crupto-currency">
       <div className="crupto-currency__block_left">
        <div className='crupto-currency__headlines'>
          <div ><GiRank1 className='crupto-currency__icon'/>
          <div className='crupto-currency__icon__rank'>{item.rank}</div>
          <div className='crupto-currency__icon__title'>rank</div>
        </div>
          <div className="crupto-currency__title"><div> {item.name}</div> <div> {item.priceUsd.slice(0,item.priceUsd.indexOf('.')+3)}</div></div>
          <div className='crupto-currency__banner'><div>Market Cap</div><div >{lineAdjustment(item.marketCapUsd.slice(0,item.marketCapUsd.indexOf('.')+3))}</div></div>
          <div className='crupto-currency__banner'><div>Volume (24Hr)</div>   <div >{lineAdjustment(item.volumeUsd24Hr.slice(0,item.volumeUsd24Hr.indexOf('.')+3))}</div></div>
          <div className='crupto-currency__banner'><div>Supply</div>  <div>{lineAdjustment(item.supply.slice(0,item.supply.indexOf('.')+3))}</div></div>
        </div>
        <div className='crupto-currency__date'>
             {day} {nameMonth[month]} {year}
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="c1" stroke="green" activeDot={{ r: 12 }} />
            </LineChart>
            <Link to="/">
                ...
                <div  className="crupto-currency__block_right">
                  <div className="crupto-currency__addition_menu">
                    <label className='crupto-currency__header'>Add currency to portfolio</label>
                     ...
                     <button className="crupto-currency__addition_menu__button" onClick={()=>onClickButton()}>add to portfolio</button>  
                  </div>
            </div>
            </div>
  );
